...
import (
	...
	"github.com/upamune/goa-playground/client"
...
func RegisterCommands(app *cobra.Command, c *client.Client) {
	...
	command = &cobra.Command{
		Use:   "sub",
		Short: `sub returns the substraction of the left and right parameters in the response body`,
	}
	tmp2 := new(SubOperandsCommand)
	sub = &cobra.Command{
		Use:   `operands ["/sub/LEFT/RIGHT"]`,
		Short: ``,
...
func (cmd *SubOperandsCommand) Run(c *client.Client, args []string) error {
	...
	if len(args) > 0 {
		...
		path = fmt.Sprintf("/sub/%v/%v", cmd.Left, cmd.Right)
	}
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
...
// RegisterFlags registers the command flags with the command line.
func (cmd *SubOperandsCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	var left int
	cc.Flags().IntVar(&cmd.Left, "left", left, `Left operand`)
	var right int
	cc.Flags().IntVar(&cmd.Right, "right", right, `Right operand`)
