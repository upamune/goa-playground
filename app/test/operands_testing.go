...
// SubOperandsOK runs the method Sub of the given controller with the given parameters.
// It returns the response writer so it's possible to inspect the response headers.
// If ctx is nil then context.Background() is used.
// If service is nil then a default service is created.
func SubOperandsOK(t goatest.TInterface, ctx context.Context, service *goa.Service, ctrl app.OperandsController, left int, right int) http.ResponseWriter {
	// Setup service
	var (
		logBuf bytes.Buffer
		resp   interface{}
	...
	rw := httptest.NewRecorder()
	u := &url.URL{
		Path: fmt.Sprintf("/sub/%v/%v", left, right),
	}
	req, err := http.NewRequest("GET", u.String(), nil)
	if err != nil {
		panic("invalid test " + err.Error()) // bug
	...
	goaCtx := goa.NewContext(goa.WithAction(ctx, "OperandsTest"), rw, req, prms)
	subCtx, err := app.NewSubOperandsContext(goaCtx, service)
	if err != nil {
		panic("invalid test data " + err.Error()) // bug
	}
